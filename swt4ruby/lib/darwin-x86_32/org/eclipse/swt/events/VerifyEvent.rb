...
module Org::Eclipse::Swt::Events
  module VerifyEventImports #:nodoc:
  ...
  class VerifyEvent < VerifyEventImports.const_get :KeyEvent
    ...
    def to_s
      string = super
      # remove trailing '}'
      return RJava.cast_to_string(string.substring(0, string.length - 1)) + " start=" + RJava.cast_to_string(@start) + " end=" + RJava.cast_to_string(@end) + " text=" + @text + "}"
    end
