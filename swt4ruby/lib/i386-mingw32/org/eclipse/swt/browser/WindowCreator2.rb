  ...
  class WindowCreator2 
    ...
    def _query_interface(riid, ppv_object)
      ...
      if (guid._equals(NsIWindowCreator::NS_IWINDOWCREATOR_IID))
        # long
        XPCOM.memmove(ppv_object, Array.typed(::Java::Int).new([@window_creator.get_address]), C::PTR_SIZEOF)
        _add_ref
        return XPCOM::NS_OK
      end
      if (guid._equals(NsIWindowCreator2::NS_IWINDOWCREATOR2_IID))
        # long
        XPCOM.memmove(ppv_object, Array.typed(::Java::Int).new([@window_creator2.get_address]), C::PTR_SIZEOF)
        _add_ref
        return XPCOM::NS_OK
      end
      # long
    ...
    def _create_chrome_window2(parent, chrome_flags, context_flags, uri, cancel, _retval)
      ...
      if (doit)
        ...
        # long
        XPCOM.memmove(_retval, Array.typed(::Java::Int).new([chrome_ptr]), C::PTR_SIZEOF)
      else
        if (!(cancel).equal?(0))
