...
module Org::Eclipse::Swt::Dnd
  module ByteArrayTransferImports
  ...
  class ByteArrayTransfer < ByteArrayTransferImports.const_get :Transfer
    ...
    def get_supported_types
      ...
      while i < types.attr_length
        ...
        data[i].attr_formatetc = FORMATETC.new
        data[i].attr_formatetc.attr_cf_format = types[i]
    ...
    def is_supported_type(transfer_data)
      ...
      i = 0
      while i < types.attr_length
        format = transfer_data.attr_formatetc
    ...
    def java_to_native(object, transfer_data)
      ...
      data = object
      size = data.attr_length
      # long
      new_ptr = OS._global_alloc(OS::GMEM_FIXED | OS::GMEM_ZEROINIT, size)
      OS._move_memory(new_ptr, data, size)
