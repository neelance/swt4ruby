...
module Org::Eclipse::Swt::Dnd
  module TableDragSourceEffectImports
  ...
  class TableDragSourceEffect < TableDragSourceEffectImports.const_get :DragSourceEffect
    ...
    def get_drag_source_image(event)
      ...
      # long
      table_image_list = OS._send_message(table.attr_handle, OS::LVM_GETIMAGELIST, OS::LVSIL_SMALL, 0)
      if (!(table_image_list).equal?(0))
        ...
        if (!OS::IsWinCE && OS::WIN32_VERSION >= OS._version(4, 10))
          if (!((table.get_style & SWT::RIGHT_TO_LEFT)).equal?(0))
            OS._set_layout(h_dc1, OS::LAYOUT_RTL | OS::LAYOUT_BITMAPORIENTATIONPRESERVED)
          end
        end
        # long
