...
module Org::Eclipse::Swt::Events
  module TraverseEventImports #:nodoc:
  ...
  class TraverseEvent < TraverseEventImports.const_get :KeyEvent
    include_class_members TraverseEventImports
    ...
    class_module.module_eval {
      const_set_lazy(:SerialVersionUID) { 3257565105301239349 }
    ...
    def to_s
      string = super
      # remove trailing '}'
      return RJava.cast_to_string(string.substring(0, string.length - 1)) + " detail=" + RJava.cast_to_string(@detail) + "}"
    end
    
    private
    alias_method :initialize__traverse_event, :initialize
  end
