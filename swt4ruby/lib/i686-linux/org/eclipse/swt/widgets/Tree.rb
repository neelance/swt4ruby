...
module Org::Eclipse::Swt::Widgets
  ...
  class Tree < TreeImports.const_get :Composite
    ...
    typesig { [::Java::Int, ::Java::Int, ::Java::Int, ::Java::Int, Array.typed(::Java::Int), ::Java::Int, ::Java::Int, ::Java::Int] }
    # long
    # long
    # long
    # long
    ...
    def copy_model(old_model, old_start, new_model, new_start, types, old_parent, new_parent, model_length)
      ...
      if (OS.gtk_tree_model_iter_children(old_model, iter, old_parent))
        # long
        # long
        old_items = Array.typed(::Java::Int).new(OS.gtk_tree_model_iter_n_children(old_model, old_parent)) { 0 }
        old_index = 0
        begin
          # long
    ...
    def create_column(column, index)
      ...
      if (!(@column_count).equal?(0))
        ...
        if ((model_index).equal?(model_length))
          ...
          # long
          # long
          copy_model(old_model, FIRST_COLUMN, new_model, FIRST_COLUMN, types, RJava.cast_to_int(0), RJava.cast_to_int(0), model_length)
    ...
    def destroy_item(column)
      ...
      if ((@column_count).equal?(0))
        ...
        # long
        # long
        copy_model(old_model, column.attr_model_index, new_model, FIRST_COLUMN, types, RJava.cast_to_int(0), RJava.cast_to_int(0), FIRST_COLUMN + CELL_TYPES)
        OS.gtk_tree_view_set_model(self.attr_handle, new_model)
        OS.g_object_unref(old_model)
        @model_handle = new_model
        create_column(nil, 0)
    ...
    def get_item(point)
      ...
      # long
      # long
      column_handle = Array.typed(::Java::Int).new(1) { 0 }
    ...
    def gtk_button_press_event(widget, event)
      ...
      OS.memmove___org_eclipse_swt_widgets_tree_5(gdk_event, event, GdkEventButton.attr_sizeof)
    ...
    def gtk_key_press_event(widget, event_ptr)
      ...
      if (OS::GTK_VERSION < OS._version(2, 2, 0))
        ...
        OS.memmove___org_eclipse_swt_widgets_tree_9(key_event, event_ptr, GdkEventKey.attr_sizeof)
        key = key_event.attr_keyval
        case (key)
        when OS::GDK_Return, OS::GDK_KP_Enter
          event = Event.new
          event.attr_item = get_focus_item
          post_event___org_eclipse_swt_widgets_tree_11(SWT::DefaultSelection, event)
        end
      end
      return result
    end
    ...
    def gtk_row_activated(tree, path, column)
      ...
      event = Event.new
      event.attr_item = item
      post_event___org_eclipse_swt_widgets_tree_13(SWT::DefaultSelection, event)
      return 0
    end
    ...
    def gtk_test_collapse_row(tree, iter, path)
      ...
      @model_changed = false
      send_event___org_eclipse_swt_widgets_tree_15(SWT::Collapse, event)
    ...
    def gtk_test_expand_row(tree, iter, path)
      ...
      old_model_changed = @model_changed
      @model_changed = false
      send_event___org_eclipse_swt_widgets_tree_17(SWT::Expand, event)
    ...
    def index_of(item)
      ...
      if ((depth).equal?(1))
        ...
        if (!(indices).equal?(0))
          ...
          OS.memmove___org_eclipse_swt_widgets_tree_21(temp, indices, 4)
          index = temp[0]
        end
      end
      OS.gtk_tree_path_free(path)
    ...
    def remove(parent_iter, start, end_)
      ...
      i = start
      while i <= end_
        OS.gtk_tree_model_iter_nth_child(@model_handle, iter, parent_iter, start)
        value = Array.typed(::Java::Int).new(1) { 0 }
        OS.gtk_tree_model_get(@model_handle, iter, ID_COLUMN, value, -1)
      ...
      if (!@ignore_size && OS._gtk_is_cell_renderer_text(cell))
        ...
        if (!(item).nil?)
          ...
          if (hooks(SWT::MeasureItem))
            ...
            if (!(width).equal?(0))
              OS.memmove___org_eclipse_swt_widgets_tree_25(content_width, width, 4)
            end
            if (!(height).equal?(0))
              OS.memmove___org_eclipse_swt_widgets_tree_27(content_height, height, 4)
            end
            ...
            send_event___org_eclipse_swt_widgets_tree_29(SWT::MeasureItem, event)
            gc.dispose
            ...
            if (!(width).equal?(0))
              OS.memmove___org_eclipse_swt_widgets_tree_31(width, content_width, 4)
            end
            if (!(height).equal?(0))
              OS.memmove___org_eclipse_swt_widgets_tree_33(height, content_height, 4)
            end
          end
        end
      end
    ...
    def renderer_render_proc(cell, window, widget, background_area, cell_area, expose_area, flags)
      ...
      if (!(item).nil?)
        if (OS._gtk_is_cell_renderer_toggle(cell) || (OS._gtk_is_cell_renderer_pixbuf(cell) && (!(column_index).equal?(0) || ((self.attr_style & SWT::CHECK)).equal?(0))))
          ...
          if (hooks(SWT::EraseItem))
            ...
            send_event___org_eclipse_swt_widgets_tree_35(SWT::EraseItem, event)
            @draw_foreground = nil
      ...
      if (!((@draw_state & SWT::BACKGROUND)).equal?(0) && ((@draw_state & SWT::SELECTED)).equal?(0))
        ...
        gc.set_background(item.get_background(column_index))
        rect = GdkRectangle.new
        OS.memmove___org_eclipse_swt_widgets_tree_37(rect, background_area, GdkRectangle.attr_sizeof)
      ...
      if (!((@draw_state & SWT::FOREGROUND)).equal?(0) || OS._gtk_is_cell_renderer_toggle(cell))
        ...
        g_class = OS.g_type_class_peek_parent(OS._g_object_get_class(cell))
        klass = GtkCellRendererClass.new
        OS.memmove___org_eclipse_swt_widgets_tree_39(klass, g_class)
      ...
      if (!(item).nil?)
        if (OS._gtk_is_cell_renderer_text(cell))
          if (hooks(SWT::PaintItem))
            rect = GdkRectangle.new
            clip_rect = GdkRectangle.new
            # long
            path = OS.gtk_tree_model_get_path(@model_handle, iter)
            OS.gtk_tree_view_get_cell_area(self.attr_handle, path, column_handle, rect)
            OS.gtk_tree_view_get_background_area(self.attr_handle, path, column_handle, clip_rect)
            OS.gtk_tree_path_free(path)
            ...
            if (!((self.attr_style & SWT::MIRRORED)).equal?(0))
              rect.attr_x = get_client_width - rect.attr_width - rect.attr_x
              clip_rect.attr_x = get_client_width - clip_rect.attr_width - clip_rect.attr_x
            end
            gc.set_clipping(clip_rect.attr_x, clip_rect.attr_y, clip_rect.attr_width, clip_rect.attr_height)
            event = Event.new
            event.attr_item = item
            event.attr_index = column_index
            event.attr_gc = gc
            ...
            event.attr_height = rect.attr_height
            event.attr_detail = @draw_state
            send_event___org_eclipse_swt_widgets_tree_41(SWT::PaintItem, event)
            gc.dispose
          end
        end
      end
    ...
    def show_item(path, scroll)
      ...
      if (depth > 1)
        ...
        OS.memmove___org_eclipse_swt_widgets_tree_43(indices, indices_ptr, indices.attr_length * 4)
        # long
