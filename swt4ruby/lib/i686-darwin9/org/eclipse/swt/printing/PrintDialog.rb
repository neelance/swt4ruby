    ...
    def open
      ...
      if (!((get_style & SWT::SHEET)).equal?(0))
        ...
        while ((@return_code).equal?(-1))
          application.run___org_eclipse_swt_printing_print_dialog_3
        end
        if (!(delegate).nil?)
          delegate.release
        end
      ...
      if (!(response).equal?(OS::NSCancelButton))
        ...
        OS.memmove___org_eclipse_swt_printing_print_dialog_5(data.attr_other_data, ns_data.bytes, data.attr_other_data.attr_length)
        @printer_data = data
      end
      print_info.release
      return data
