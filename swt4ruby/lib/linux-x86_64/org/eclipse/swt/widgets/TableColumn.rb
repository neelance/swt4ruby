...
module Org::Eclipse::Swt::Widgets
  ...
  class TableColumn < TableColumnImports.const_get :Item
    ...
    def gtk_clicked(widget)
      ...
      if (post_event)
        post_event___org_eclipse_swt_widgets_table_column_3(double_click ? SWT::DefaultSelection : SWT::Selection)
      end
      return 0
    end
    ...
    def gtk_event_after(widget, gdk_event)
      event = GdkEvent.new
      OS.memmove___org_eclipse_swt_widgets_table_column_5(event, gdk_event, GdkEvent.attr_sizeof)
      case (event.attr_type)
      when OS::GDK_BUTTON_PRESS
        gdk_event_button = GdkEventButton.new
        OS.memmove___org_eclipse_swt_widgets_table_column_7(gdk_event_button, gdk_event, GdkEventButton.attr_sizeof)
    ...
    def pack
      ...
      if (!((@parent.attr_style & SWT::VIRTUAL)).equal?(0))
        i = 0
        while i < @parent.attr_items.attr_length
        ...
        if (OS.gtk_tree_model_get_iter_first(@parent.attr_model_handle, iter))
          begin
            width = Math.max(width, @parent.calculate_width(self.attr_handle, iter))
