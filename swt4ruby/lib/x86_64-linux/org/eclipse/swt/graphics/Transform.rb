...
module Org::Eclipse::Swt::Graphics
  ...
  class Transform < TransformImports.const_get :Resource
    ...
    def initialize(device, m11, m12, m21, m22, dx, dy)
      ...
      @handle = Array.typed(::Java::Double).new(6) { 0.0 }
      if ((@handle).nil?)
    ...
    def get_elements(elements)
      ...
      elements[0] = (@handle[0]).to_f
      elements[1] = (@handle[1]).to_f
      elements[2] = (@handle[2]).to_f
      elements[3] = (@handle[3]).to_f
      elements[4] = (@handle[4]).to_f
      elements[5] = (@handle[5]).to_f
    end
    ...
    def is_identity
      ...
      m = Array.typed(::Java::Float).new(6) { 0.0 }
      get_elements(m)
      return (m[0]).equal?(1) && (m[1]).equal?(0) && (m[2]).equal?(0) && (m[3]).equal?(1) && (m[4]).equal?(0) && (m[5]).equal?(0)
    end
    ...
    def transform(point_array)
      ...
      dx = Array.typed(::Java::Double).new(1) { 0.0 }
      dy = Array.typed(::Java::Double).new(1) { 0.0 }
