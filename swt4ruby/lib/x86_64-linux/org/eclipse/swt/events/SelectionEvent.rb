...
module Org::Eclipse::Swt::Events
  ...
  class SelectionEvent < SelectionEventImports.const_get :TypedEvent
    ...
    def to_s
      string = super
      # remove trailing '}'
      return (string.substring(0, string.length - 1)).to_s + " item=" + (@item).to_s + " detail=" + (@detail).to_s + " x=" + (@x).to_s + " y=" + (@y).to_s + " width=" + (@width).to_s + " height=" + (@height).to_s + " stateMask=" + (@state_mask).to_s + " text=" + @text + " doit=" + (@doit).to_s + "}"
    end
